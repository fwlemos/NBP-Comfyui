"""
Error handling utilities for the ComfyUI Nano Banana node pack.
...
def handle_api_error(exc: Exception) -> str:
    ...
    if "403" in msg or "permission" in msg or "forbidden" in msg:
        raise RuntimeError(
            "[Nano Banana] Access denied. Your API key may lack permission "
    ...
    if "429" in msg or "rate" in msg or "quota" in msg or "resource_exhausted" in msg:
        raise RuntimeError(
            ...
            "Nano Banana allows ~20 images/minute. Wait and retry."
        ) from exc

    # --- Bad request / validation ---
    if "400" in msg or "invalid" in msg or "bad request" in msg:
        raise RuntimeError(
            f"[Nano Banana] Invalid request: {exc_str}. "
    ...
    if "500" in msg or "503" in msg or "internal" in msg:
        raise RuntimeError(
            f"[Nano Banana] Google API server error. "
    ...
    if "timeout" in msg or "timed out" in msg:
        raise RuntimeError(
            "[Nano Banana] Request timed out. "
            "The API may be overloaded — try again."
        ) from exc

    if "connection" in msg or "network" in msg:
        raise RuntimeError(
            "[Nano Banana] Could not connect to the Google API. "
            "Check your internet connection."
        ) from exc

    # --- Catch-all ---
    raise RuntimeError(
        f"[Nano Banana] Unexpected error: {exc_str}"
    ) from exc
